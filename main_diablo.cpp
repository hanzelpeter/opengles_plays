...
bool LoadTexture()
{
    ...
    pFile = fopen("demon_color.bmp", "rb");
    if(pFile == NULL)
        return false;
...
void Render()
{
    ...
    vec4 target = vec4(0,0,0,0);
   vec4 scale = vec4(0.05,0.05,0.05,0);
    ...
    //glDrawArrays(GL_TRIANGLES, arraycount/2, arraycount/2);
...
int main(void) {
  ...
  CreateProgram();
 rs.ninja.LoadFromOBJ("diablo_low.obj");
//Dump();
    // load the texture
    glActiveTexture(GL_TEXTURE0);
